	...
	$.conversion = function(obj) {
			...
			$.conversion._appendIframe = function() {
				if ($('#ifr-conversion').size() == 0) {
					$('body').append($.conversion.ifr);
					$.conversion.child = $('#ifr-conversion').seamless({
						loading : ''
					});
					$.conversion.child
							.receive(function(data, event) {
								console
										.log('conversion recieving url data.. : ');
								console.log(data);

								if (data.hoopla) {
									console
											.log('conversion requesting hoopla url data.. : ');
								...
								if (data.destroy) {
									console
											.log('conversion sending destroy message.. :');
									// PubSub.publish('destroyUrlService',
									// data.url);
									$.conversion.destroy();
								}
							});
			...
			$.conversion._show = function() {
				...
				setTimeout(function() {
					...
					$('#ifr-conversion').css('border-bottom',
							'15px solid #073163');
					$('#ifr-conversion').show();
				}, 2500);
			...
			$.conversion.init = function(data) {
				...
				var getUrlServiceResult = function(msg, data) {
					console
							.log("conversion parent getUrlServiceResult recieving data : ");
					console.log(msg, data);
				};
				var getUrlServiceResultToken = PubSub.subscribe(
						'getUrlServiceResult', getUrlServiceResult);
